...
vec4 bg1(vec2 p,float r) {
	return get(p,r, 0, -0.1, -0.05);
}
vec4 bg2(vec2 p,float r) {
	return get(p,r, 0, -0.4, -0.3);
}
vec4 bg3(vec2 p,float r) {
	return get(p,r, 0, -0.1, -0.3);
}
...
void main() {
	vec2 p = -1.0 + 2.0 * uvl;
	float r = clamp(sqrt(dot(p,p)), 0,0.9999);
	float f = (1.0-sqrt(1.0-r))/(r) / 2.0;

	vec4 c = (bg1(p,f) * bg2(p,f) *2)* bg3(p,f) *3;

	c.rgb /= pow(r*20, 1.0-activity);
