...
Engine::Engine(const std::string& title, Configuration cfg)
  : _asset_manager(std::make_unique<asset::Asset_manager>(cfg.get("exe").get_or_throw(), title)),
    ...
    _sdl(),
    _graphics_ctx(std::make_unique<renderer::Graphics_ctx>(title, _configuration->get_as("g.width", 1920),
                                                           _configuration->get_as("g.height", 1080),
                                                           _configuration->get_as("g.fullscreen", true))),
